...
export function RealEstateCalculator() {
	...
	const { formatCurrencySafe, abbreviateNumber } =
	...
	const [operatingCostsPercent, setOperatingCostsPercent] =
		useState(1.5);
	...
	const cashFlowData = useMemo(() => {
		...
		// Today's cash flow
		const todayCashFlow =
			monthlyRent - monthlyMortgagePayment - monthlyOps;

		// Cash flow at different years (rent grows, mortgage stays fixed)
		const cashFlowAtYear = (year: number) => {
			const futureRent =
				monthlyRent * Math.pow(1 + rentGrowth / 100, year);
			const futureOps =
				monthlyOps * Math.pow(1 + rentGrowth / 100, year); // Ops grow with inflation
	...
	const wealthChartData = useMemo(() => {
		...
		for (let year = 0; year <= mortgageTerm; year++) {
			...
			const currentPropertyValue =
				propertyValue * Math.pow(1 + appreciation / 100, year);
			...
			const futureRent =
				monthlyRent * Math.pow(1 + rentGrowth / 100, year);
			const futureOps =
				monthlyOps * Math.pow(1 + rentGrowth / 100, year);
			const yearCashFlow =
				year < mortgageTerm
					? (futureRent - monthlyMortgagePayment - futureOps) *
						12
	...
	const milestones = useMemo((): Milestone[] => {
		...
		return years
			.map((year) => {
				const dataPoint =
					crossoverChartData.find((d) => d.year === year) ||
					crossoverChartData[year];
				return {
					year,
			...
			.filter(
				(m, i, arr) =>
					arr.findIndex((x) => x.year === m.year) === i,
			);
	...
	const summary = useMemo(() => {
		...
		const annualizedROI =
			(Math.pow(1 + totalWealth / downPayment, 1 / mortgageTerm) -
				1) *
			100;

		// Leverage calculation
		const annualAppreciationDollars =
			(propertyValue * appreciation) / 100;
		const leverageReturn =
			(annualAppreciationDollars / downPayment) * 100;
	...
	> = {
		propertyValue: {
			title: t.propertyValue_info,
			description: t.propertyValue_desc,
		},
		downPayment: {
			title: t.downPayment_info,
			description: t.downPayment_desc,
		},
		monthlyRent: {
			title: t.monthlyRent_info,
			description: t.monthlyRent_desc,
		},
		appreciation: {
			title: t.appreciation_info,
			description: t.appreciation_desc,
		},
		mortgageRate: {
			title: t.mortgageRate_info,
			description: t.mortgageRate_desc,
		},
		mortgageTerm: {
			title: t.mortgageTerm_info,
			description: t.mortgageTerm_desc,
		},
		rentGrowth: {
			title: t.rentGrowth_info,
			description: t.rentGrowth_desc,
		},
		operatingCosts: {
			title: t.operatingCosts_info,
			description: t.operatingCosts_desc,
		},
	};
	...
	const HelpButton = ({ sliderKey }: { sliderKey: SliderInfoKeys }) => (
		<Button
			...
			onClick={(e) => {
				e.stopPropagation();
				openInfoDrawer(
					sliderInfo[sliderKey].title,
					sliderInfo[sliderKey].description,
				);
			}}
		>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
							...
							<CardContent className="p-5">
								...
								<div className="space-y-2">
									<div className="flex justify-between items-center">
										...
										<span
											className={`font-bold ${cashFlowData.today >= 0 ? 'text-amber-800 dark:text-amber-200' : 'text-red-600 dark:text-red-400'}`}
										>
											{formatCurrencySafe(
												roundNumber(
													cashFlowData.today,
						...
						<Card className="bg-gradient-to-br from-sky-50/80 to-sky-100/50 dark:from-sky-950/40 dark:to-sky-900/20 backdrop-blur-md border border-sky-200/50 dark:border-sky-800/30 shadow-lg overflow-hidden">
							<CardContent className="p-5">
								...
								<div className="space-y-2">
									<div className="flex justify-between items-center">
										<span className="text-sm text-sky-800/70 dark:text-sky-300/70">
											{t.tenantPaysMortgage}
										</span>
										<span className="font-bold text-sky-800 dark:text-sky-200">
											{formatCurrencySafe(
												roundNumber(
													equityBuildingPerMonth,
												),
											)}
											{t.perMonth}
										</span>
								...
								<div className="space-y-2">
									<div className="flex justify-between items-center">
										<span className="text-sm text-emerald-800/70 dark:text-emerald-300/70">
											{appreciation}% {t.perYear}
										</span>
										<span className="font-bold text-emerald-800 dark:text-emerald-200">
											{formatCurrencySafe(
												roundNumber(
													appreciationPerMonth,
												),
											)}
											{t.perMonth}
										</span>
						...
						<CardContent className="p-6">
							<div className="flex flex-col md:flex-row items-center justify-between gap-4">
								<div className="text-center md:text-start">
									<p className="text-sm text-violet-700/70 dark:text-violet-300/70 mb-1">
										{t.totalWealthBuilt} (
										{mortgageTerm} {t.yearsLabel})
									</p>
							...
							<div className="flex items-start gap-3">
								...
								<div>
									...
									<p className="text-sm text-amber-800/80 dark:text-amber-300/80 mt-1">
										{appreciation}% {t.appreciationOn}{' '}
										{formatCurrencySafe(propertyValue)}{' '}
										={' '}
										<span className="font-semibold">
											{formatCurrencySafe(
												roundNumber(
													summary.annualAppreciationDollars,
												),
											)}
											{t.perYear}
										</span>
										<br />
										{t.butYouOnlyInvested}{' '}
										{formatCurrencySafe(downPayment)}
										...
										<br />
										<span className="text-amber-900 dark:text-amber-100 font-bold">
											{t.thatsReturn.replace(
												'{n}',
												summary.leverageReturn.toFixed(
													1,
												),
											)}
										</span>
										<span className="text-xs ms-2 opacity-70">
											({t.bankMoneyWorksForYou})
										</span>
									</p>
								</div>
							</div>
								...
								{milestones.map((milestone) => (
									...
									>
										<span className="text-xs text-gray-500 dark:text-gray-400">
											{t.year} {milestone.year}
										</span>
										<span
											className={`text-lg font-bold ${getMilestoneColor(milestone.status)}`}
										>
											{formatCurrencySafe(
												roundNumber(
													milestone.cashFlow,
										...
										<span
											className={`text-xs ${getMilestoneColor(milestone.status)}`}
										>
											{getMilestoneLabel(
												milestone.status,
											)}
