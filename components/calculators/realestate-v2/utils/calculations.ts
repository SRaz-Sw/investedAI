 ...
 *
 * NOTE: `purchasePrice` input is treated as MARKET VALUE.
 * The actual purchase price = marketValue × (1 - belowMarketPercent/100)
 * This way, increasing "below market %" means you pay less, affecting loan amount.
 */
export function calculateDerivedValues(
	...
	// Actual purchase price = market value × (1 - below market %)
	// e.g., $100K market value at 30% below market = $70K purchase price
	const actualPurchasePrice =
		marketValue * (1 - belowMarketPercent / 100);

	// Instant equity = what it's worth - what you pay
	const instantEquity = marketValue - actualPurchasePrice;

	// Down payment and loan based on actual purchase price
	const downPayment = actualPurchasePrice * (downPaymentPercent / 100);
	const loanAmount = actualPurchasePrice - downPayment;
...
export function calculateMonthlyExpenses(
	...
	const management =
		currentRent * (inputs.propertyManagementPercent / 100);
	const maintenance =
		(currentValue * (inputs.maintenancePercent / 100)) / 12;
...
export function calculateYear1Results(
	...
	const withLeverageNetMonthly =
		monthlyRent - monthlyExpenses.total - monthlyMortgage;
	const withLeverageAnnualCashFlow = withLeverageNetMonthly * 12;
	const withLeverageAppreciation =
		marketValue * (appreciationRate / 100);
	...
	for (let month = 0; month < 12 && balance > 0; month++) {
		...
		const principalPayment = Math.min(
			monthlyMortgage - interestPayment,
			balance
		);
...
export function generateProjectionData(
	...
	const monthlyAppreciationFactor = Math.pow(
	...
	for (let month = 0; month <= TOTAL_MONTHS; month++) {
		...
		const equityPercent =
			currentValue > 0 ? (equity / currentValue) * 100 : 0;
		...
		// === CASH FLOW ===
		const effectiveMortgage =
			mortgageBalance > 0 ? monthlyMortgage : 0;
